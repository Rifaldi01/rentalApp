...
@if($item)
    @section('title', strtoupper('edit alat'))
@else
    @section('title', strtoupper('tambah alat'))
@endif
@section('content')
    <div class="card">
        <div class="card-head">
        ...
        <div class="card-body p-4">
            <form action="{{$url}}" method="POST" enctype="multipart/form-data" id="myForm">
                ...
                <div class="mb-2">
                    ...
                    <input type="text" name="name" class="form-control" placeholder="Masukan Nama Alat" value="{{isset($item) ? $item->name : null}}">
                </div>
                <div class="mt-3 mb-2">
                    <label for="single-select-field" class="form-label">Kategori</label>
                    <select name="cat_id" class="form-select" id="single-select-clear-field" data-placeholder="Pilih Kategori">
                ...
                <div class="mt-3 mb-2">
                    <label class="col-form-label">Gamabar</label>
                    @if (isset($item) && $item->image)
                        <div class="mt-3">
                            <h6>Masukan Gamabar:</h6>
                            <div class="row">
                                @foreach (json_decode($item->image) as $image)
                                    <div class="col-md-2">
                                        <img src="{{ asset('images/item/' . $image) }}" alt="Image" class="img-thumbnail mb-2">
                                        <button type="button" class="btn btn-danger btn-sm delete-image" data-image="{{ $image }}">Hapus</button>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                ...
                <div class="mt-3">
                    ...
                    @if(isset($item))
                        <a href="{{route('admin.item.index')}}" class="btn btn-warning float-end me-2"><i class="bx bx-undo"></i>Kembali</a>
                    @else
                        <a href="{{route('admin.item.index')}}" class="btn btn-warning float-end me-2"><i class="bx bx-list-ul"></i>Daftar Alat</a>
                    @endif
                </div>
            </form>
        </div>
